...
class GoogleAuthenticator
{
     ...
     */
    public function getCode(string $secret, int|float|null $timeSlice = null): string
    {
        if ($timeSlice === null) {
    ...
    protected function _base32Decode($secret)
    {
        ...
        for ($i = 0; $i < count($secret); $i = $i + 8) {
            ...
            for ($j = 0; $j < 8; ++$j) {
                $x .= str_pad(base_convert(strval(@$base32charsFlipped[@$secret[$i + $j]]), 10, 2), 5, '0', STR_PAD_LEFT);
            }
            $eightBits = str_split($x, 8);
            for ($z = 0; $z < count($eightBits); ++$z) {
                $binaryString .= (($y = chr(intval(base_convert($eightBits[$z], 2, 10)))) || ord($y) == 48) ? $y : '';
            }
        }
