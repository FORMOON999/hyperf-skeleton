...
class ArrayHelper
{
     ...
     * @param array|Closure|int|string $key
     ...
     * @param null|array|string $path the path of where do you want to write a value to `$array`
     ...
     * @param null|Closure|string $key the column name or anonymous function which result will be used to index the array
     * @param null|Closure[]|string|string[] $groups the array of keys, that will be used to group the input array
    ...
    public static function multisort(array &$array, $key, $direction = SORT_ASC, $sortFlag = SORT_REGULAR): void
    {
        ...
        foreach ($keys as $i => $iKey) {
            $flag = $sortFlag[$i];
            $args[] = static::getColumn($array, $iKey);
            $args[] = $direction[$i];
            $args[] = $flag;
        }
     ...
     *
     * @return null|array the public member variables of the object or null if not object given
     */
